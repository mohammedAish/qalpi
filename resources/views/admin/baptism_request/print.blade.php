...
<div class="container">
      <div class="top ">
         <div class="d-flex text">
            <h4 class="pt-5">رقم التعميد :  <span class="px-2">{{$baptism_request->id }}</span></h4>
            <h3 class="title text-center px-2 py-2">تعميد   <br>علي حساب الجمعية</h3>
         </div>
         <div class="d-flex mt-5 pt-5 ml-5">
             <h3 class="pr-5 mx-5">التاريخ : {{$baptism_request->created_at->format('d/m/Y')}}</h3>
             <h3 class=" pl-5 ml-5">رقم الطلب : {{$baptism_request->app_id }}</h3>
      ...
      <div class="body">
          <div class="row">
              ...
              <div class="px-3 mx-0 col-11 pt-4 mt-5" style="border: 1px solid #000">
                  <h4>اسم المستفيد : <span class="px-2">{{$baptism_request->patientsـname }}</span></h4>
                  <div class="py-3 d-flex justify-content-between">
                    <h4>رقم الهوية  : <span class="px-2">{{$baptism_request->patientsـid }}</span></h4>
                        <h4 class="pr-5 mr-5">رقم الجوال  : <span class="px-2">{{$baptism_request->patientsـphone }}</span></h4>
                  </div>
                  <div class="d-flex justify-content-between">
                    <h4>رقم الملف  : <span class="px-2">{{$baptism_request->app_id }}</span></h4>
                        <h4 class="pr-5 mr-5"> التشخيص : <span class="px-2">{{$baptism_request->typeـdiagnosis }}</span></h4>
                  </div>
              </div>
          </div>
          <div class="row body2 ">
            <div class="mx-0 px-0 col-1 pt-4 mt-4 ">
              ...
              <div class="text-center  px-3 mx-0 col-11 pt-4 mt-5" style="border: 1px solid #000">
                <h2 class="text-left">السادة / {{$baptism_request->serviceـprovider }} <span class="px-3">المحترمين</span></h2>
                <h5 class="text-left pt-4">السلام عليكم ورحمة الله وبركاته</h5>
                <p class="pt-5 mt-3">بالاشارة الي قبول الحالة من المركز اعتمدو علاج المستفيد الموضحة بياناته أعلاه بمبلغ وقدره <strong class="px-2"> {{$baptism_request->serviceـprice }} ريال فقط</strong>وسيصلكم الشيك بعد تقديم مستندات العلاج من طرفكم باذن الله تعالي , كما  نأمل  تزويدنا الشيك بعد تقديم مستندات العلاج من طرفكم باذن الله تعالي . كما نأمل تزودينا  بالتقرير الطبي بعد بعد انتهاء العملية مع القيمة  االمالية ليتم صرف  المبلغ لكم </p>
                <p>علما بأن الجمعية لا تتحمل أي مصاريف اضافية خارج نطاق هذا التعميد </p>
                <p>للتواصل مع الرقم <strong>{{$baptism_request->user_phone }}</strong></p>
      ...
      <div class="my-5">
        <a id="proceed" href="{{route('baptism_request.index')}}" class="btn btn-secondary">الغاء</a>
    </div>
    </div>
            </div>
